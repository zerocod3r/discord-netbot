...
class Fun_Commands(commands.Cog):
    ...
    async def cat(self, ctx):
        """ Posts a random cat """
        await self.randomimageapi(ctx, 'https://api.alexflipnote.dev/cats', 'file', token=self.content_fetch_api)

    @commands.command()
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def dog(self, ctx):
        """ Posts a random dog """
        await self.randomimageapi(ctx, 'https://api.alexflipnote.dev/dogs', 'file', token=self.content_fetch_api)

    @commands.command(aliases=["bird"])
    @commands.cooldown(rate=1, per=1.5, type=commands.BucketType.user)
    async def birb(self, ctx):
        """ Posts a random birb """
        await self.randomimageapi(ctx, 'https://api.alexflipnote.dev/birb', 'file', token=self.content_fetch_api)
    ...
    async def supreme(self, ctx, *, text: commands.clean_content(fix_channel_mentions=True)):
        ...
        await self.api_img_creator(ctx, f"https://api.alexflipnote.dev/supreme?text={inputText}&{darkorlight}", "supreme.png", token=self.content_fetch_api)
        ...
        async with ctx.channel.typing():
            ...
            try:
                r = await http.get(
                    ...
                    no_cache=True, headers={"Authorization": self.content_fetch_api}
                )
            except aiohttp.ClientConnectorError:
                return await ctx.send("The API seems to be down...")
            except aiohttp.ContentTypeError:
    ...
    @commands.command(aliases=['society'])
